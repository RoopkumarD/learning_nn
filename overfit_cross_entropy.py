...
class Network:
    ...
    def cost_function(self, last_layer: np.ndarray, y: np.ndarray, sample_num: int):
        return (-1 / sample_num) * np.sum(
            y * np.log(last_layer) + (1 - y) * np.log(1 - last_layer)
        )

    def train(
        self, training_x: np.ndarray, training_y: np.ndarray, epoch: int, l2: float
    ):
        ...
        for ep in range(epoch):
            ...
            delta = activations[-1] - training_y
            ...
            sample_num = len(training_x)
            factor = 1 - ((self.learn_rate * l2) / sample_num)

            self.weights = [
                factor * w - ((self.learn_rate / sample_num) * dw)
                for w, dw in zip(self.weights, delta_w)
            ]
            self.bias = [
                b - ((self.learn_rate / sample_num) * db)
                for b, db in zip(self.bias, delta_b)
            ]
            print(
                f"Epoch {ep} -> Cost {self.cost_function(activations[-1], training_y, sample_num)}"
            )
...
if __name__ == "__main__":
    ...
    m.train(training_x, training_y, 10000, 0)

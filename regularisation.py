...
class Network:
    ...
    def train(
        self, training_x: np.ndarray, training_y: np.ndarray, epoch: int, rg: float
    ):
        ...
        for ep in range(epoch):
            ...
            self.weights = [
                ((1 - (self.learn_rate * rg / len(training_x))) * w)
                - ((self.learn_rate / len(training_x)) * dw)
...
if __name__ == "__main__":
    ...
    m.train(training_x, training_y, 10000, 5)
    for x in training_x:

...
class Network:
    ...
    def train(self, train_data: dict, epoch: int, learn_rate: float, batch_num: int):
        ...
        for ep in range(epoch):
            ...
            # log cost function
            # https://sootlasten.github.io/2016/softmax-backprop-derivation/
            print(
                f"Epoch {ep} -> Cost {np.sum(-np.log(pred[np.argmax(train_data_y, axis=1), np.arange(train_data_y.shape[0])])) / length_of_train}"
            )
